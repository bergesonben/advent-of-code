...
function initMap(input: string): [MapSnapshot, number, number] {
	const lines = input.split("\n");
	const mapWidth = lines[0].length;
	const mapHeight = lines.length;
	const startMap: MapSnapshot = [];
	...
	return [startMap, mapWidth, mapHeight];
}

function getNeighbors(cell: CellSnapshot, maps: MapSnapshot[]): CellSnapshot[] {
	...
	const dirs = directions(cell.row, cell.col, maps[0].length, maps[0][0].length);
...
function nextMap(map: MapSnapshot): MapSnapshot {
	const newMap = Array(map.length).fill(false).map(x => Array(map[0].length));
	for (let row = 0; row < map.length; row++) {
		for (let col = 0; col < map[0].length; col++) {
			...
			for (let blizzard of curr.blizzards) {
				const [nextRow, nextCol] = getBlizzardNextSpot(blizzard, [row,col], map.length, map[0].length);
...
function getBlizzardNextSpot(blizzard: Blizzard, [row, col]: Coor, mapHeight: number, mapWidth: number): Coor {
	switch (blizzard) {
		case '<':
...
function directions(row: number, col: number, mapHeight: number, mapWidth: number): [number,number][] {
...
async function p2022day24_part1(input: string, ...params: any[]) {
	const [startMap, mapWidth, mapHeight] = initMap(input);
